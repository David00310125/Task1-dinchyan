...
class StatusesController(Statuses):
    ...
    statuses = []
    #список (массив) в котором хранятся словари со значениями атрибутов класса Statuses
    #методы CRUD
    #метод создания
    def add(self):
    ...
    #В цикле переменной element_list при каждой итерации (повторяющееся действия) будет присвоено число от 0 до индекса последнего элемента    # range(len(self.statuses)) - определяет длинну списка
    #уловие используется для нахождения в каждом элементе списка id из параметра метода delete    #если такой id найден, будет удалён элемент из списка
    def delete(self,id):
